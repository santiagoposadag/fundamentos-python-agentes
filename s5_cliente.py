...
# CONCLUSION: En este capítulo se comprobó que un script de Python puede comunicarse con el servidor FastAPI utilizando
# la librería requests, sin necesidad de usar el navegador.
...
# CONCLUSION: En este capítulo se ejecutó el flujo completo de comunicación cliente-servidor desde un script de Python.
# Se verificó que el servidor estaba activo, se registró un agente, se consultó su información, se envió un mensaje y
# se revisó la bandeja del destinatario, todo mediante peticiones HTTP.
# Al repetir las mismas acciones en Swagger UI, se confirmó que los datos coincidían, lo que demuestra que tanto el navegador
# como el script interactúan con el mismo servidor y la misma base de datos.
...
# -----------------------------------------------------------#
print(enviar_mensaje_http("Orion", "Atlas", "Mensaje enviado desde cliente HTTP"))