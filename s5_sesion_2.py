...
@app.get("/")
def inicio():
    return {"status": "aquí", "mensaje": "Bienvenido al sistema de agentes"}
...
# CONCLUSION: Se comprobó que FastAPI convierte funciones normales en endpoints HTTP mediante decoradores
# como @app.get("/"). La respuesta retornada en forma de diccionario se transforma automáticamente en JSON.
# Además, Swagger UI genera documentación interactiva basada en el código, lo que permite probar los endpoints
# sin necesidad de herramientas externas. El flag --reload facilita el desarrollo al reiniciar el servidor
# automáticamente cuando se detectan cambios en el archivo.
...
# CONCLUSION: Se verificó que los path parameters permiten enviar información dinámica en la URL (por ejemplo, el nombre del agente).
# Cuando el agente existe, el servidor responde con código 200 OK y retorna sus datos en formato JSON. Cuando no existe,
# se lanza una HTTPException con código 404 Not Found, lo que demuestra un manejo correcto de errores HTTP.
# Esto confirma que FastAPI permite estructurar respuestas claras y coherentes según el resultado de la operación.
...
# CONCLUSION: Se comprobó que el método POST permite enviar datos en el cuerpo (body) de la petición en formato JSON.
#  FastAPI utiliza modelos Pydantic para validar automáticamente los datos recibidos. Cuando los datos son correctos,
#  el servidor responde normalmente con código 200. Cuando los datos son inválidos (por ejemplo, enviar texto en lugar de un entero),
#  FastAPI retorna automáticamente un error 422, indicando qué campo es incorrecto. Esto demuestra la importancia de la
#  validación automática y cómo reduce la necesidad de validaciones manuales.
